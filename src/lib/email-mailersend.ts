// MailerSend 邮件服务
// 免费 3000 封/月，支持 GitHub 登录

const MAILERSEND_API_KEY = process.env.MAILERSEND_API_KEY || '';
const MAILERSEND_SENDER_EMAIL = process.env.MAILERSEND_SENDER_EMAIL || 'noreply@yourdomain.com';
const MAILERSEND_SENDER_NAME = process.env.MAILERSEND_SENDER_NAME || 'PDF AI Chat';
...
async function sendEmailViaMailerSend(
  ...
  return data?.[0]?.message_id || 'unknown';
}
...
export async function sendVerificationEmail(
  ...
  try {
    ...
    console.log('[MailerSend] 发送验证邮件到:', email);

    const messageId = await sendEmailViaMailerSend(
      email,
      displayName,
    ...
    console.log('✓ MailerSend 验证邮件发送成功:', messageId);
    return { success: true, messageId };
  } catch (error: any) {
    console.error('✗ MailerSend 邮件发送失败:', error.message);
    return {
      success: false,
...
export async function sendPasswordResetEmail(
  ...
  try {
    ...
    console.log('[MailerSend] 发送密码重置邮件到:', email);
    ...
    console.log('✓ MailerSend 密码重置邮件发送成功:', messageId);
    return { success: true, messageId };
  } catch (error: any) {
    console.error('✗ MailerSend 邮件发送失败:', error.message);
    return {

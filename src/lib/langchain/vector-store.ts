...
export async function createVectorStore(
  ...
  const cache = getVectorStoreCache();
  console.log(
    `[VectorStore] Creating vector store for ${pdfId} with ${documents.length} documents`
  );
...
export async function createVectorStoreFromChunks(
  ...
  try {
    ...
    const vectorStore = await createVectorStore(pdfId, documents, true, userId);
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        console.error(
          `[VectorStore] ⚠️ API Key issue detected! Please check your ALIBABA_API_KEY or QWEN_API_KEY`
        );
...
export async function searchSimilarDocuments(
  ...
  if (isPineconeConfigured) {
    ...
    try {
      const results = await searchPineconeVectors(pdfId, query, k, userId);
      if (results.length > 0) {
  ...
  if (!vectorStore) {
    ...
    try {
      ...
      if (chunks && chunks.length > 0) {
        console.log(
          `[VectorStore] ✓ Found ${chunks.length} chunks in Redis, recreating vector store...`
        );
        const documents = chunks.map(
          (chunk) =>
            new Document({
  ...
  if (!vectorStore) {
    console.warn(
      `[VectorStore] No vector store found for ${pdfId}. Available IDs:`,
      getVectorStoreIds()
    );
    return [];
  }
  ...
  results.forEach(([doc, score], idx) => {
    console.log(
      `[VectorStore] Result ${idx + 1}: score=${score.toFixed(4)}, preview="${doc.pageContent.substring(0, 50)}..."`
    );
  });
  ...
  if (filteredResults.length < results.length) {
    console.log(
      `[VectorStore] Filtered ${results.length - filteredResults.length} low-score results (threshold: ${threshold})`
    );
  }
  ...
  if (filteredResults.length === 0 && results.length > 0) {
    console.log(
      `[VectorStore] All results filtered, returning top ${Math.min(2, results.length)} anyway`
    );

---
description: 通过提出最多 5 个高度针对性的澄清问题并将答案编码回规范中，识别当前功能规范中未充分说明的区域。
handoffs: 
  - label: 构建技术计划
    agent: speckit.plan
...
注意：此澄清工作流程预计在调用 `/speckit.plan` 之前运行（并完成）。如果用户明确表示他们正在跳过澄清（例如，探索性尖峰），您可以继续，但必须警告下游返工风险增加。

执行步骤：

1. 从仓库根目录运行一次 `{SCRIPT}`（组合 `--json --paths-only` 模式 / `-Json -PathsOnly`）。解析最小 JSON 有效负载字段：
   - `FEATURE_DIR`
   - `FEATURE_SPEC`
...
3. 生成（内部）优先级候选澄清问题队列（最多 5 个）。不要一次全部输出。应用这些约束：
    ...
    - 仅包括答案会实质性影响架构、数据建模、任务分解、测试设计、UX 行为、运营准备或合规性验证的问题。
    - 确保类别覆盖平衡：尝试首先覆盖影响最大的未解决类别；避免在单个高影响区域（例如，安全态势）未解决时提出两个低影响问题。
...
4. 顺序提问循环（交互式）：
    ...
    - 对于多项选择问题：
       ...
       | 简短 | 提供不同的简短答案（<=5 个单词）（仅在适合自由形式替代方案时包含）|
...
5. 每个接受的答案后的集成（增量更新方法）：
    ...
    - 对于此会话中的第一个集成答案：
       - 确保存在 `## Clarifications` 部分（如果缺少，则在规范模板的最高级别上下文/概述部分之后创建它）。
    ...
    - 然后立即将澄清应用于最合适的部分：
       ...
       - 边缘情况/负面流程 → 在边缘情况/错误处理下添加新项目符号（或者如果模板为其提供占位符，则创建此类子部分）。
       - 术语冲突 → 在规范中规范化术语；仅在必要时保留原始术语，方法是添加 `（以前称为"X"）` 一次。
    - 如果澄清使早期的模糊陈述无效，请替换该陈述而不是重复；不要留下过时的矛盾文本。
...
8. 报告完成（在提问循环结束或提前终止后）：
   ...
   - 更新规范的路径。
   - 触及的部分（列出名称）。
   - 覆盖摘要表，列出每个分类法类别，状态为：已解决（曾经是部分/缺失并已解决）、推迟（超过问题配额或更适合计划）、清晰（已经足够）、未解决（仍然是部分/缺失但影响较低）。
   - 如果仍有未解决或推迟的问题，建议是继续 `/speckit.plan` 还是稍后在计划后再次运行 `/speckit.clarify`。
   - 建议的下一个命令。
